...
let pulls
try {
  pulls = JSON.parse(execFileSync('gh', ['api', `repos/${repository}/commits/${releaseSha}/pulls`, '--header', 'Accept: application/vnd.github+json'], { encoding: 'utf8' }))
} catch {
  throw new Error('GitHub could not verify merged pull requests for the release commit')
}
if (!Array.isArray(pulls) || !pulls.some((pull) => pull?.merged_at && pull.base?.ref === defaultBranch && pull.base?.repo?.full_name === repository && pull.merge_commit_sha === releaseSha))
